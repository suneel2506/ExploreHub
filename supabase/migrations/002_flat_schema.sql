-- ============================================================================
-- Explore Hub — Migration 002: Flat Schema
-- ============================================================================
-- DESTRUCTIVE: Drops old hierarchical tables and recreates flat schema.
-- Safe on fresh install. Run in Supabase SQL Editor.
-- ============================================================================

-- ============================================================================
-- 0. CLEANUP — Drop old views and tables (order matters for FK deps)
-- ============================================================================
...
-- Generated FTS column (name + state + district + city + category)
...
COMMENT ON TABLE public.places IS 'Curated places imported from OpenStreetMap or other sources.';

-- ============================================================================
-- 2. CUSTOM_PLACES — user-added
-- ============================================================================
...
COMMENT ON TABLE public.custom_places IS 'User-created places not in the official catalog.';
...
-- ============================================================================
-- 7. VIEW — v_user_stats (flat schema version)
-- ============================================================================
...
SELECT
    ...
    COALESCE((
        SELECT COUNT(*) FROM public.visited_places v
        WHERE v.user_id = p.id AND v.place_id IS NOT NULL
    ), 0) AS visited_count,

    COALESCE((
        SELECT COUNT(DISTINCT pl.country)
        FROM public.visited_places v
        JOIN public.places pl ON pl.id = v.place_id
        WHERE v.user_id = p.id
    ), 0) AS countries_explored,

    COALESCE((
        SELECT COUNT(DISTINCT pl.state)
        FROM public.visited_places v
        JOIN public.places pl ON pl.id = v.place_id
        WHERE v.user_id = p.id AND pl.state IS NOT NULL
    ), 0) AS states_explored,

    COALESCE((
        SELECT COUNT(DISTINCT pl.district)
        FROM public.visited_places v
        JOIN public.places pl ON pl.id = v.place_id
        WHERE v.user_id = p.id AND pl.district IS NOT NULL
    ), 0) AS districts_explored,
    ...
    COALESCE((
        SELECT COUNT(*) FROM public.custom_places cp
        WHERE cp.user_id = p.id
    ), 0) AS custom_places_count

FROM public.profiles p;

COMMENT ON VIEW public.v_user_stats IS 'Aggregated stats per user (flat schema).';
...
CREATE POLICY "cp_update" ON public.custom_places FOR UPDATE TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "vp_update" ON public.visited_places FOR UPDATE TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "wl_update" ON public.wishlist FOR UPDATE TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "mem_update" ON public.memories FOR UPDATE TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "med_update" ON public.media FOR UPDATE TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
...
-- Allow authenticated users to upload to photos bucket under their own folder
...
-- ============================================================================
-- Done! Run import-osm.cjs next to populate places.
-- ============================================================================
